...
const UPLOAD_FEE_ROBUX = 5;    // charged to the uploader for clothing uploads (faces are free - see FACE_CATEGORY check
                                // in /catalog/upload below) - waived entirely for admins. Credited to the RETROBLOX
                                // system account (see RETROBLOX_SYSTEM_ACCOUNT_USERNAME) rather than just vanishing.
const RETROBLOX_SYSTEM_ACCOUNT_USERNAME = 'retroblox';
...
app.post('/catalog/upload', catalogImageUpload.single('image'), (req, res) => {
    try {
        ...
        // Faces are always free to upload. Admins upload everything free too. Everyone
        // else pays the flat UPLOAD_FEE_ROBUX for clothing (T-Shirts/Shirts/Pants) and
        // accessories - that fee goes to the RETROBLOX system account below rather than
        // just disappearing.
        const isFaceUpload = category === FACE_CATEGORY;
        const chargeUploadFee = !isFaceUpload && !isAdmin;

        if (chargeUploadFee && (account.robux || 0) < UPLOAD_FEE_ROBUX) {
        ...
        if (chargeUploadFee) {
            ...
            // Credit the fee to the RETROBLOX system account instead of it just vanishing
            // - mirrors how a catalog purchase pays the item's creator (see
            // /accounts/:username/inventory/buy below). Guard against the uploader
            // somehow being the RETROBLOX account itself (isAdmin already skips the fee
            // for it, so this is just defensive) and against that account not existing.
            const retrobloxAccount = findAccountByUsername(RETROBLOX_SYSTEM_ACCOUNT_USERNAME);
            if (retrobloxAccount && retrobloxAccount.username !== account.username) {
                retrobloxAccount.robux = (retrobloxAccount.robux || 0) + UPLOAD_FEE_ROBUX;
            }
        }
        saveAccountsIndex();
...
// ---- Change a catalog item's description (Creations -> item's "..." settings) ----
// Every catalog item requires a non-empty description at upload time (see
// /catalog/upload above), so this mirrors that - it can be edited, but not cleared.
app.post('/catalog/:id/description', (req, res) => {
    const item = findCatalogItemById(req.params.id);
    if (!item) return res.status(404).json({ error: 'not-found', message: 'That catalog item does not exist.' });
    if (!requireCatalogItemOwner(req, res, item)) return;
    ...
    const description = (req.body.description || '').toString().trim();
    if (!description) return res.status(400).json({ error: 'invalid-description', message: 'Description cannot be empty.' });
    item.description = description.slice(0, 1000);
